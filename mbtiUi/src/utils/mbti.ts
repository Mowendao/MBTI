...
export function calculateMBTI(
  ...
  // 动态计算每维度最大可能分数
  const getMaxScore = (dim: string): number => {
    if (questionsPerDimension && questionsPerDimension[dim]) {
      return questionsPerDimension[dim] * 3; // 每题 score 范围 [-3, +3]
    }
    return 15; // 默认 5 题 * 3
  };
  ...
  const calculatePercentage = (score: number, maxScore: number): number => {
    ...
    return Math.min(100, Math.round((absoluteScore / maxScore) * 100));
  };

  const ePercentage = calculatePercentage(eiScore, getMaxScore('EI'));
  const iPercentage = 100 - ePercentage;
  const sPercentage = calculatePercentage(snScore, getMaxScore('SN'));
  const nPercentage = 100 - sPercentage;
  const tPercentage = calculatePercentage(tfScore, getMaxScore('TF'));
  const fPercentage = 100 - tPercentage;
  const jPercentage = calculatePercentage(jpScore, getMaxScore('JP'));
