...
public class BatchController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<Batch>>> getAll() {
        return ResponseEntity.ok(ApiResponse.success(repository.findAll()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<Batch>> getById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.success(repository.findById(id).orElseThrow()));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<Batch>> create(@RequestBody Batch entity) {
        entity.setId(null);
        return ResponseEntity.ok(ApiResponse.success("创建成功", repository.save(entity)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Batch>> update(@PathVariable Long id, @RequestBody Batch updated) {
        Batch entity = repository.findById(id).orElseThrow();
        ...
        if (updated.getStartDate() != null) entity.setStartDate(updated.getStartDate());
        if (updated.getEndDate() != null) entity.setEndDate(updated.getEndDate());
